...
void DesktopMultiWindowPlugin::HandleMethodCall(
  ...
  if (method_call.method_name() == "createWindow") {
    ...
    auto window_id = method_call.arguments()->LongValue();
    const auto maximized = MultiWindowManager::Instance()->IsMaximized(window_id);
    result->Success(flutter::EncodableValue(maximized));
    return;
  } else if (method_call.method_name() == "toggleMaximize") {
    auto window_id = method_call.arguments()->LongValue();
    const auto maximized = MultiWindowManager::Instance()->ToggleMaximize(window_id);
    result->Success(flutter::EncodableValue(maximized));
    return;
