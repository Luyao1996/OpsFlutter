// ★本地补丁（相对 pub.dev xterm 4.0.0 唯一改动，见 _openInputConnection）：
//   TextInputConfiguration 补传 viewId。Flutter 3.24+ 的多视图文本输入要求
//   IME attach 时携带目标 FlutterView 的 id，缺失时引擎报
//   "Could not set client, view ID is null"，IME 连接建立失败 ⇒ 字符输入
//   全部丢失（Tab/Enter/粘贴走硬件键路径不受影响）。
//   上游未修复：https://github.com/TerminalStudio/xterm.dart/issues/207
//   写法对齐官方 EditableText（flutter/lib/src/widgets/editable_text.dart 的
//   _createTextInputConfiguration：viewId: View.of(context).viewId）。
  ...
  void _openInputConnection() {
    ...
    if (hasInputConnection) {
      ...
      final config = TextInputConfiguration(
        // ★本地补丁：见文件头注释。缺 viewId 时 Flutter 3.24+ 无法建立 IME 连接。
