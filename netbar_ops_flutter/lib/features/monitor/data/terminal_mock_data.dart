...
class TerminalMockData {
  ...
  static List<Map<String, dynamic>> hardware(int terminalId) {
    final rng = _rng(terminalId, 'hardware');
    String pick(List<String> items) => items[rng.nextInt(items.length)];
    String percent(double v) => '${v.toStringAsFixed(0)}%';
    ...
    final memGb = 16 + rng.nextInt(3) * 16;
    final diskGb = 512 + rng.nextInt(3) * 512;

    return [
      {
        'name': '处理器 (CPU)',
        'details': [
          {'label': '型号', 'value': cpuModel},
          {'label': '核心/线程', 'value': pick(['6C/12T', '8C/16T', '12C/24T'])},
          {'label': '当前占用', 'value': percent(10 + rng.nextDouble() * 70)},
        ],
      },
      {
        'name': '内存 (RAM)',
        'details': [
          {'label': '容量', 'value': '$memGb GB'},
          {'label': '频率', 'value': pick(['2666 MHz', '3200 MHz', '3600 MHz'])},
          {'label': '当前占用', 'value': percent(20 + rng.nextDouble() * 70)},
        ],
      },
      {
        'name': '显卡 (GPU)',
        'details': [
          {'label': '型号', 'value': gpuModel},
          {'label': '显存', 'value': pick(['6 GB', '8 GB', '12 GB'])},
          {'label': '当前占用', 'value': percent(5 + rng.nextDouble() * 60)},
        ],
      },
      {
        'name': '存储 (Disk)',
        'details': [
          {'label': '容量', 'value': '$diskGb GB'},
          {'label': '类型', 'value': pick(['NVMe SSD', 'SATA SSD', 'HDD'])},
          {'label': '当前占用', 'value': percent(10 + rng.nextDouble() * 70)},
        ],
      },
      {
        'name': '网络 (Network)',
        'details': [
          {'label': '网卡', 'value': pick(['Realtek PCIe GBE', 'Intel I219-V', 'Killer E2500'])},
          {'label': 'IP', 'value': '192.168.${10 + rng.nextInt(30)}.${10 + rng.nextInt(200)}'},
          {'label': '速率', 'value': pick(['1000 Mbps', '100 Mbps'])},
        ],
      },
      {
        'name': '主板 (Board)',
        'details': [
          {'label': '厂商', 'value': pick(['ASUS', 'MSI', 'Gigabyte', 'Colorful'])},
          {'label': '型号', 'value': pick(['B460M', 'B550M', 'H510M', 'B660M'])},
          {'label': 'BIOS', 'value': 'v${1 + rng.nextInt(4)}.${rng.nextInt(10)}.${rng.nextInt(100)}'},
        ],
      },
    ];
  }
