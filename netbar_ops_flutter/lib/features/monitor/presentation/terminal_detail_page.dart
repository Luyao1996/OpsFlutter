...
class _TerminalDetailPageState extends ConsumerState<TerminalDetailPage>
  ...
  /// 点击"笨鸟远程"（老版，入口已隐藏）按钮：不再弹"看/控"选择框，
  /// 与 V3 保持一致——服务器（主/副）用「控」，终端用「看」。
  Future<void> _handleWebRTCButtonTap(Terminal terminal) async {
    await _openWebRTCRemote(terminal, viewOnly: !terminal.isKeyDevice);
  }

  /// 点击"笨鸟远程V3"按钮：不再弹"看/控"选择框，按设备类型直接决定模式。
  /// 服务器（主/副）用「控」打开，终端用「看」打开，判定见 [Terminal.isKeyDevice]。
  Future<void> _handleWebRTCV3ButtonTap(Terminal terminal) async {
    await _openWebRTCRemoteV3(terminal, viewOnly: !terminal.isKeyDevice);
  }
